...
const API_URL =
  process.env.NEXT_PUBLIC_API_URL || 'https://api.uricreative.com:8448';
...
export const api = {
  ...
  auth: {
    ...
    sendVerificationOtp: async (email: string) => {
      return fetchClient('/api/auth/email/send-otp', {
        method: 'POST',
        body: JSON.stringify({ email }),
        skipAuth: true,
      });
    ...
    verifyEmailOtp: async (email: string, otp: string) => {
      return fetchClient('/api/auth/email/verify-otp', {
        method: 'POST',
  ...
  assignments: {
    ...
    delete: (id: string) =>
      fetchClient(`/api/assignments/${id}`, { method: 'DELETE' }), // Admin only
  },
  ...
  cohorts: {
    ...
    update: (id: string, data: ApiPayload) =>
      fetchClient(`/api/cohorts/${id}`, {
        method: 'PATCH',
        body: JSON.stringify(data),
      }), // Admin only
    deactivate: (id: string) =>
      fetchClient(`/api/cohorts/${id}/deactivate`, {
        method: 'PATCH',
      }), // Admin only
    delete: (id: string) =>
