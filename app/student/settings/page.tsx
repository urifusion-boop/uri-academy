...
export default function Settings() {
    ...
    const fetchProfile = async () => {
      try {
        ...
        if (data.user) {
          ...
          setEmailVerified(!!userAny.emailVerified);
  ...
  const handleSendOtp = async () => {
    ...
    setOtpLoading(true);
    try {
      await api.auth.sendVerificationOtp(user.email);
      setOtpSent(true);
      setOtpValue('');
      startResendCooldown();
  ...
  const handleVerifyOtp = async () => {
    ...
    try {
      await api.auth.verifyEmailOtp(user.email, otpValue);
      setEmailVerified(true);
      setOtpSent(false);
      setOtpValue('');
                    ...
                    <div>
                      ...
                      <div className="relative">
                        <input
                          ...
                          className="input-field bg-gray-50 pr-28"
                          disabled
                        />
                        {emailVerified ? (
                          <span className="absolute inset-y-0 right-0 pr-3 flex items-center gap-1 text-green-600 text-xs font-medium pointer-events-none">
                          ...
                          <button
                            ...
                            disabled={otpLoading}
                            className="absolute inset-y-0 right-0 pr-3 flex items-center text-xs font-medium text-brand-600 hover:text-brand-500 disabled:opacity-50"
                          >
                      ...
                      {otpSent && !emailVerified && (
                        <div className="mt-3 p-4 bg-blue-50 border border-blue-100 rounded-lg space-y-3">
                          <p className="text-sm text-blue-800">
                            Enter the 6-digit code sent to your email. It
                          ...
                          <div className="flex gap-2">
                            <input
                              ...
                              placeholder="000000"
                              className="input-field w-32 text-center tracking-widest font-mono"
                            />
                            <button
                              type="button"
                          ...
                          <button
                            ...
                            className="text-xs text-brand-600 hover:text-brand-500 disabled:text-gray-400 disabled:cursor-not-allowed transition-colors"
                          >
                            {resendCooldown > 0
